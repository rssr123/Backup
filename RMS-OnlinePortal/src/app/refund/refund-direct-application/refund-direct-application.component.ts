...
@Component({
  selector: 'app-refund-direct-application',
  templateUrl: './refund-direct-application.component.html',
  styleUrls: ['./refund-direct-application.component.scss']
})
export class RefundDirectApplicationComponent {
  ...
  refundrequestmsg: String | null = null;
  ...
  fetchRttform(): void {
    this.isLoading = true;
    // fetch the refund request message
    const refundRequest = this.refundHistModel.find(item => item.action === 'Refund Request');

    if (refundRequest) {
      this.refundrequestmsg = refundRequest.msg;
    }
  ...
  stateHistory() {
    ...
    if (this.rtt_status === '' || this.rtt_status === null || this.rtt_status === 'BE' || this.rtt_status === 'RR') {
      this.isRefundable = true;
    }
    if (this.rtt_app_no !== '' && this.rtt_app_no !== null) {
      this.isRttAppNo = true;
      if (this.rtt_status === 'RR') {
        this.isRttAppNo = false;
      }
    }
  ...
  async handleRequestRefund(): Promise<void> {
    ...
    if (selectedItems.length > 0 && this.BankInfoForm.valid) {
      this.isLoading = true;
      ...
      const url = environment.apiUrl + '/api/refundl/v1/addrttwf_da';
      const headers = new HttpHeaders({
        Authorization: environment.authKey,
        'Content-Type': 'application/json',
      });
     ...
     const response: any = await this.http.post(url, body, { headers }).toPromise();
      ...
      this.alertMessage = 'Please select at least one item for a refund or Fill in the bank info.';
      this.alertClass = 'alert alert-danger PA-alert-box';
      this.showInsertAlert = true;
  ...
  loadPostcode() {
    // 1) keep your valueChanges binding so that
    //    any *new* user selection will still populate City/State
    this.BankInfoForm.get('postcode')?.valueChanges
    ...
    this.http.post(url, {}, { headers }).subscribe(
      (response: any) => {
        ...
        // 3) **now** that uniqueCities/States are ready,
        //    re‑apply the existing postcode (from your fetchRttform patch)
        const existingPC = this.BankInfoForm.get('postcode')?.value;
        if (existingPC) {
          this.onPostcodeChange(existingPC);
        }
      },
      (error) => {
