...
@Component({
  selector: 'app-billing-adjustment-review',
  templateUrl: './billing-adjustment-review.component.html',
  styleUrls: ['./billing-adjustment-review.component.scss']
})
export class BillAdjustmentReviewComponent {
  ...
  async fetchBillInfo()  {
    ...
    this.http.post(url, Body, { headers }).subscribe(
      (response: any) => {
        ...
        /*if(this.currentUser == this.billInfo.created_by && this.billInfo.history[0].msg_type == 'QAR' && this.billInfo.bil_wf_status == 'WF-AN')  //Owner of the bill to return FA query
            this.hideActions = false;*/
                
        this.isUser = this.currentUser == this.billInfo.created_by || this.currentName == this.billInfo.created_by;
        if(this.billInfo.bil_wf_status == 'WF-AN' && this.billInfo.history[0].msg_type == 'QAR')
            this.hideActions = false;
        if(!this.isUser)
  ...
  submitForm(){
    var url = environment.apiUrl + '/api/billing/v1/querybilladj';
