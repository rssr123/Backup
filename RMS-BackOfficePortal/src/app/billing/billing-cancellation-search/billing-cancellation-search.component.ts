  ...
  navigateScreen(item: billinglisting){
    ...
    if(bil_wf_status.includes('WF-CN')){
      ...
      if((this.isUser && (action == 'Query Requester' || item.bil_wf_status.includes(' Q'))) || this.permAllowApp  > 0)
        this.router.navigate(['/billing-cancellation-search/billing-cancellation-approval'], { queryParams: { billing_no }});
      else
    ...
    //else if((this.isUser || this.roles!.includes('FINANCEADMIN')) && this.isCancelAllowed(bil_wf_status, item.unpaid))
      ...
      this.router.navigate(['/billing-cancellation-search/billing-cancellation'], { queryParams: { billing_no }});
    else
      this.router.navigate(['/billing-cancellation-search/billing-details'], { queryParams: { billing_no }});    
  }
