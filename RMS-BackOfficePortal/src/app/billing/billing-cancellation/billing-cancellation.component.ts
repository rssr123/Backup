...
@Component({
  selector: 'app-billing-cancellation',
  templateUrl: './billing-cancellation.component.html',
  styleUrls: ['./billing-cancellation.component.scss']
})
export class BillingCancellationComponent {
  ...
  isValidToCan: boolean = false;
  isFinPopulating: boolean = false;
  isCanceled: boolean = false;

  permCheck = perm.Billing_Cancellation_Request;
  ...
  updateBillCancel(): void {
    const url = environment.apiUrl + '/api/billing/v1/reqbillcan';
    this.isLoading = true;
    const headers = new HttpHeaders({
      Authorization: environment.authKey,
      'Content-Type': 'application/json',
    ...
    const body: any = {
      ...
      i_remark: 'Billing Cancellation Request'
    };
      ...
      this.http
        ...
        .then((response) => {
          ...
          setTimeout(() => {
            ...
            this.router.navigate(['/billing-cancellation-search']);
            location.href = 'billing-cancellation-search';
            return;
          }, 2000);
        })
        .catch((error) => {
  ...
  loadPermission() {
    this.authService.checkUserRole(this.currentUser as string, this.permCheck)
        ...
        this.permAllow = this.permCheckReturnString.includes(perm.Billing_Cancellation_Request) ? 1 : 0;
