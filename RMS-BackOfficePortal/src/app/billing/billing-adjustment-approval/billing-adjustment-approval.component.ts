...
@Component({
  selector: 'app-billing-adjustment-approval',
  templateUrl: './billing-adjustment-approval.component.html',
  styleUrls: ['./billing-adjustment-approval.component.scss']
})
export class BillAdjustmentApprovalComponent {
  ...
  permCheck = perm.Billing_Adjustment_Request + ',' + perm.Billing_Adjustment_Approval;
  ...
  async ngOnInit() {
    ...
    if(this.billNo != null){
      this.loadPermission();
      this.fetchBillInfo();
      this.fetchOldItemsInfo();
    }
  }

  async fetchOldItemsInfo(){
    const headers = new HttpHeaders({
      Authorization: environment.authKey,
      'Content-Type': 'application/json',
    });
    const url = environment.apiUrl + '/api/billing/v1/getbilitems';
    ...
    this.http.post(url, Body, { headers }).subscribe(
      (response: any) => {
        this.old_billing_items = response.data;
        console.log(this.old_billing_items)
        if(this.old_billing_items != null && this.old_billing_items.length > 0)
          for(const item of this.old_billing_items)
            this.old_total_amount += item.final_amt;
      },
      (error) => {
        console.error(error);
      }
    ...
    if(this.isBillUnauth){
      this.old_billing_items = [];
      this.old_total_amount = 0;
    }
  }

  getOldBillingItem(mftpk: number): any {
    if (!mftpk) {
      return '';
    }
    const entity = this.old_billing_items.find((option) => option.mft_pk === mftpk);
    return entity;
  }
    ...
    this.http.post(url, Body, { headers }).subscribe(
      (response: any) => {
        ...
        /*if(((this.currentUser != this.billInfo.created_by && this.roles!.includes('FINANCEADMIN')) || this.roles!.includes('FINANCEADMIN'))
          && this.billInfo.bil_wf_status == 'WF-AN' && this.billInfo.history[0].msg_type != 'QAR'){ //FA (not registrar!)
            this.hideActions = false;
            this.queryOnly = false;
            this.isApprover = true;
        }
        else if(this.currentUser == this.billInfo.created_by && this.billInfo.history[0].msg_type == 'QAR' && this.billInfo.bil_wf_status == 'WF-AN')  //Owner of the bill to return FA query
            this.hideActions = false;*/

        this.isUser = this.currentUser == this.billInfo.created_by || this.currentName == this.billInfo.created_by;

        if(this.billInfo.bil_wf_status.includes('WF-AN'))
            this.hideActions = false;

        if(this.roles!.includes('FINANCEADMIN') || this.permAllowApp > 0)
            this.isApprover = true;
        
        if((this.billInfo.history.length > 0 && this.billInfo.history[0].msg_type == 'QAR') || this.billInfo.has_query > 0){
          this.queryOnly = true;
          if(!this.isUser)
  ...
  submitForm(){
    ...
    if(this.selectedDecision == 'aprv')
      url += '/api/billing/v1/aprovebilladj';
    else if(this.selectedDecision == 'rjct')
      url += '/api/billing/v1/rejectbilladj';
    else if (this.selectedDecision == 'query')
      url += '/api/billing/v1/querybilladj';
  ...
  loadPermission() {
    this.authService.checkUserRole(this.currentUser as string, this.permCheck)
      .subscribe((response: any) => {
          ...
          this.permAllowQuery = this.permCheckReturnString.includes(perm.Billing_Adjustment_Request) ? 1 : 0;
