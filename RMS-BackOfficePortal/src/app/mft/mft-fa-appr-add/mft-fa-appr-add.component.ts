...
@Component({
  selector: 'app-mft-fa-appr-add',
  templateUrl: './mft-fa-appr-add.component.html',
  styleUrls: ['./mft-fa-appr-add.component.scss'],
  animations: [fadeInOut]
})
export class MftFaApprAddComponent implements OnInit {
  ...
  permRequestAddMFTwithRequesterFormFA = perm.Master_Fee_Table_Request_Add_MFT_with_Requester_Form_FA; // all the perm_cd for this module seperated with comma
  permRequestAddMFTwithRequesterFormFAAllow = ""; // variable to store allowed permission for the user
  ...
  async ngOnInit() {
    ...
    if (this.wfId !== undefined) {
      ...
      this.decisionOption = [ //put here to prevent user from choose decision if url is manual enter
      ...
      else { //this.statusFromAssigned === "P-FA" //means await financeadmin approval, need to further check status in below
        const tempQuery = await this.getworkflowhistory_status("P-FA");
        this.queryBackTo = tempQuery.assign_to;
        this.queryFromStatus = tempQuery.status;

        if (this.queryFromStatus === "P-RHOD") { //means task is approved from requester HOD therefore need insert and populate from r fields
          // this.inputDetails = true;
          this.populateFromRFields = true;
          this.decisionDetails = true;
          this.actionType = "Add";
          //    await this.populateFeeGroupId();
  ...
  async updateMFTWF() {
    ...
    if (this.decision !== 'RJ-FA') { //if decision=reject, no need to validate
      if (this.previousInsertFeeDetailId !== this.feeDetailId) {//if previous fee detail id is same as current fee detail id, means no change
        ...
        console.log("previous insert dfee detail id is" + this.previousInsertFeeDetailId)
        console.log("current fee detail id is" + this.feeDetailId)
      }
    }
 ...
 formatInputllStartDay(llStartDayRef: any): void {
    ...
    try {
      ...
      if (response.header.statusCode === '00') {
        ...
        if (this.populateFromRFields === false) {  //means query from finance hod
          ...
          else { //check if this.mftwfs[0].fee_grp_id exist in current feeGroups
  ...
  checkPermission() {
    this.authService.checkUserRole(this.authService.username, this.permRequestAddMFTwithRequesterFormFA)
      .subscribe(
        (response: any) => {
          this.permRequestAddMFTwithRequesterFormFAAllow = response.data;
          console.log("this.permRequestAddMFTwithRequesterFormFAAllow " + this.permRequestAddMFTwithRequesterFormFAAllow);
          this.permListAllow = this.permRequestAddMFTwithRequesterFormFAAllow.includes(perm.Master_Fee_Table_Request_Add_MFT_with_Requester_Form_FA) ? 1 : 0;
