...
@Component({
  selector: 'app-mft-fa-fa-edit',
  templateUrl: './mft-fa-fa-edit.component.html',
  styleUrls: ['./mft-fa-fa-edit.component.scss'],
  animations: [fadeInOut]
})
export class MftFaFaEditComponent implements OnInit {
  ...
  currentFeeDetailIPk: number | string | null = null;
  ...
  permRequestEditMFTFA = perm.Master_Fee_Table_Request_Edit_MFT_FA; // all the perm_cd for this module seperated with comma
  permRequestEditMFTFAAllow = ""; // variable to store allowed permission for the user
  ...
  .pipe(
    ...
    if (this.wfId !== undefined) {
      ...
      this.loadDataSupDoc();
      this.loadDataHist();
      await this.populateRequestedValueForm(); //populate for pre fill fields
    ...
    else { //this page is navigated from edit button from mft details page


      this.inputDetails = true;
      this.actionType = "Edit";
      this.populateAppover();
      // this.populateCurrentValueForm();
      // this.populateRequestedValueForm(); //need put after await
  ...
  async insertMasterFeeTableWorkFlow(): Promise<boolean> {
    ...
    let formattedPromoStartDt = this.datePipe.transform(this.promoStartDate, 'yyyy-MM-dd') || "";
    let formattedPromoEndDt = this.datePipe.transform(this.promoEndDate, 'yyyy-MM-dd') || "";
    ...
    this.feeDetailId = this.currentFeeDetailId;
    // this.feeDetailPk = this.currentFeeDetailIPk;
    ...
    if (formattedPromoStartDt) {
      body.i_promo_startdt = formattedPromoStartDt;
    }

    if (formattedPromoEndDt) {
      body.i_promo_enddt = formattedPromoEndDt;
    }
    ...
    body.i_action = "Request Edit-FIN";
  ...
  //validation start //edit no need check wf since allow multiple user to submit at the same time
  ...
  async checkDuplicateMFTWF() {
    ...
    try {
      ...
      if (response.header.statusCode === '01') {
        ...
        this.error = true;
        this.errorMessages.push('Fee Detail PK is duplicate Master Fee Table work flow.');
        return true;
      }
    } catch (error) {
      this.error = true;
  ...
  async populateRequestedValueForm(): Promise<void> {
    ...
    try {
      ...
      if (response.header.statusCode === '00') {
        ...
        // this.feeDetailId = this.mftwfs[0].fee_detail_id === null ? null : this.mftwfs[0].fee_detail_id.toString(); because is loaded from currentform function
  ...
  cancel() {
    ...
    if (this.wfId !== undefined){
      ...
      this.router.navigate(['/mft-item-detail'], { state: { fee_detail_pk } });
    }
  }
 ...
 formatInputllStartDay(llStartDayRef: any): void {
  ...
  /*async getworkflowhistory_ast(status: string): Promise<string> {
    ...
    try {
      ...
      if (response.header.statusCode === '00') {
        return response.data[0].assign_to; // Data found
      } else {
        this.error = true;
  ...
  async populateCurrentValueForm(): Promise<void> {
    ...
    try {
      ...
      if (response.header.statusCode === '00') {
        ...
        this.currentFeeDetailIPk = this.mfts[0].fee_detail_pk;
        ...
        this.currentIsPub = this.mfts[0].isPub;
  ...
  checkPermission() {
    this.authService.checkUserRole(this.authService.username, this.permRequestEditMFTFA)
      .subscribe(
        (response: any) => {
          this.permRequestEditMFTFAAllow = response.data;
          console.log("this.permRequestEditMFTFAAllow " + this.permRequestEditMFTFAAllow);
          this.permListAllow = this.permRequestEditMFTFAAllow.includes(perm.Master_Fee_Table_Request_Edit_MFT_FA) ? 1 : 0;
