...
@Component({
  selector: 'app-mft-req-form-edit',
  templateUrl: './mft-req-form-edit.component.html',
  styleUrls: ['./mft-req-form-edit.component.scss'],
  animations: [fadeInOut]
})
export class MftReqFormEditComponent implements OnInit {
  ...
  //insert
  tempRFeeDetNm: { pk: number, id: string, name: string } | null = null;
  ...
  //  {{ item.fee_detail_id }} : {{ item.fee_detail_nm_e }} update whenever scroll after 100, typing and change source system
  searchTerm: string = ''; // Store search term for querying mfts
  loading: boolean = false; // To prevent multiple API calls
  searchTermSubject: Subject<string> = new Subject<string>(); // Use Subject to emit search terms
  dropDownSize100 = 100;
  currentSearchTerm = '';
  ...
  permEditMFTRequesterFormR = perm.Master_Fee_Table_Edit_MFT_Requester_Form_R; // all the perm_cd for this module seperated with comma
  permEditMFTRequesterFormRAllow = ""; // variable to store allowed permission for the user
  ...
  async ngOnInit() {
    ...
    // await this.populateFeeDetailID();
     // Subscribe to the search term subject and trigger API calls with debounce
    this.loadMfts();
  ...
  .pipe(
    ...
    tap(term => {
      this.currentSearchTerm = term ?? ''; //  STORE TERM
      this.page = 1;                       //  RESET PAGE
      this.mfts = [];                      //  CLEAR OLD DATA
    }),
    switchMap(term => this.loadMfts(term))
  )
  .subscribe();
  ...
  async insertMasterFeeTableWorkFlow(): Promise<boolean> {
    ...
    /*let tempFeeDetailPK: string | null = null;
    if (this.tempRFeeDetNm !== null) {
      // Concatenate id and name and assign it to ss
      this.rFeeDetNm = this.tempRFeeDetNm.id + ' - ' + this.tempRFeeDetNm.name;
      tempFeeDetailPK = this.tempRFeeDetNm.pk.toString();
    }
    else {
      tempFeeDetailPK = null;
    }*/

    await this.getFeeDetailIDAndNmByPK();
    this.rFeeDetNm = this.tempFeeDetailId + ' : ' + this.tempFeeDetailNmEn;
  ...
  async populateForm(): Promise<void> {
    ...
    try {
      ...
      if (response.header.statusCode === '00') {
        ...
        // if (this.mftwfs[0].fee_detail_pk === null || this.mftwfs[0].fee_detail_pk === undefined) { //because ngmodel is feeGroupId
        //   this.tempFeeDetailPK = null;
        // }
        // else {
        //   if (this.mfts.some(mfts => mfts.fee_detail_pk === this.mftwfs[0].fee_detail_pk)) {
        //     this.tempFeeDetailPK = this.mftwfs[0].fee_detail_pk;
        //   }
        //   else {
        //      this.tempFeeDetailPK = null;
        //   }
        // }
       const pk = this.mftwfs[0].fee_detail_pk;

        if (pk) {
          const selectedMft = await this.loadMftByPk(pk);

          if (selectedMft) {
            // Ensure dropdown contains it
            if (!this.mfts.some(m => m.fee_detail_pk === pk)) {
              this.mfts = [selectedMft, ...this.mfts];
            }

            // Select it
            this.tempFeeDetailPK = pk;
            console.log("The tempFeeDetailPK is" +this.tempFeeDetailPK )
          } else {
            this.tempFeeDetailPK = null;
          }
        } else {
 ...
 */
  ...
  // async populateFeeDetailID(pk: number): Promise<void> {
  ...
  checkPermission() {
    this.authService.checkUserRole(this.authService.username, this.permEditMFTRequesterFormR)
      .subscribe(
        (response: any) => {
          this.permEditMFTRequesterFormRAllow = response.data;
          console.log("this.permEditMFTRequesterFormRAllow " + this.permEditMFTRequesterFormRAllow);
          this.permListAllow = this.permEditMFTRequesterFormRAllow.includes(perm.Master_Fee_Table_Edit_MFT_Requester_Form_R) ? 1 : 0;
...
//
  // This function is triggered when the user selects a new source system
  onSourceSystemChange(): void {
    // Reset page number for new source system selection
    this.page = 1;
    // Clear previous fee details
    this.mfts = [];
    //  CLEAR Fee Detail box
    this.tempFeeDetailPK = null;
    // Fetch mfts again based on the newly selected source system
    this.loadMfts(this.searchTerm);
  }

  // This function is triggered when the user clicks on the Fee Detail dropdown
  onFeeDetailNameDropdownOpen(): void {
    // Fetch the fee details whenever the dropdown is opened
    this.loadMfts(this.searchTerm);
  }

  // This function is triggered by the typeahead input field
onSearchTermChange(term: string): void {
  ...
  // Function to load MFTs (Master Fee Table) based on current page and search term
  async loadMfts(searchTerm: string = ''): Promise<any> {
    if (this.loading) return; // Avoid multiple requests while loading
    this.loading = true;
    ...
    const Body = {
      i_page: this.page,
      i_size: this.dropDownSize100, // cl said use 100
      i_status: Systemstatus.Active,
      i_ss_cd: this.rSelectedSourceSystemCodes.length ? this.rSelectedSourceSystemCodes.join(',') : null, //convert array to string
      i_searchTerm: searchTerm // Pass the search term for filtering
    };
    ...
    try {
      ...
      if (response.header.statusCode === '00') {
        if (this.page === 1) {
          this.mfts = response.data; // Clear previous results
        } else {
          this.mfts = [...this.mfts, ...response.data]; // Append new data
        }
      } else {
        console.error('Error fetching mfts:', response);
      }
    } catch (error) {
      console.error('Error retrieving the master fee table:', error);
    } finally {
      this.loading = false;
    }
  }

  // Triggered when the user scrolls to the bottom of the ng-select dropdown
 loadMoreItems(): void {
  if (this.loading) return;

  this.page++;
  this.loadMfts(this.currentSearchTerm); // NEVER use empty term
}

//below is use to populate fee detail id after user query back to requester using fee_detail_pk
//diffent getFeeDetailIDAndNmByPK because only need retrun one item
async loadMftByPk(pk: number): Promise<MFT | null> {
  ...
  try {
    ...
    if (response.header.statusCode === '00' && response.data?.length) {
      return response.data[0]; //  return ONE item
    }

    return null;
  } catch (error) {
    console.error('Failed to load MFT by pk', error);
    return null;
  }
}
