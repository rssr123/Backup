...
import { MFTWFDoc, Param, User } from '../../core/models/entity';
...
@Component({
  selector: 'app-mft-item-task-list',
  templateUrl: './mft-item-task-list.component.html',
  styleUrls: ['./mft-item-task-list.component.scss']
})
export class MftItemTaskListComponent {
  ...
  async ngOnInit() {
    ...
    if (tempshow_requester_table !== null && tempshow_requester_table !== 'null') {
      ...
      this.fromRoute = history.state.from_route;
    

    console.log("From route is " + this.fromRoute)
    //this.wfId = history.state.wf_id;
    // this.pendingStatus = history.state.status__From_Assigned;
    // this.taskId = history.state.task_id;
    //this.feeDetailPk = history.state.fee_detail_pk;
  ...
  async checkPermission(): Promise<void> {
    ...
    const bodyPermTCDList: any = {
      i_username: this.authService.username,
  ...
  cancel() {
       //1 = to created task
       //2 = to mft item detail
       //3 = to master task list
    if (this.fromRoute === 2) {

      const fee_detail_pk = this.feeDetailPk;
      //const fee_detail_id = this.feeDetailId;
      console.log("fee_detail_pk is " + fee_detail_pk);
    this.router.navigate(['/mft-item-detail'], { state: { fee_detail_pk } });
    }
    else if(this.fromRoute === 1){
