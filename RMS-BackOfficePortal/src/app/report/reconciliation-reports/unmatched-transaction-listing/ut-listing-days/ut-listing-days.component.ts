...
@Component({
  selector: 'app-ut-listing-days',
  templateUrl: './ut-listing-days.component.html',
  styleUrls: ['./ut-listing-days.component.scss']
})
export class UtListingDaysComponent implements OnInit {
  ...
  constructor(
    ...
    this.bsConfig = {
      ...
      maxDate: this.today // Set maxDate to today's date
    };
  ...
  viewSelected(item: any) {
    ...
    this.router.navigate(['/ut-listing-transactions'], { state: { dummydate } });
  ...
  async exportSelected() {
    ...
    console.log('Body.i_period_key:', Body.i_period_key); // Debug log to check the value of i_period_key

    // Ensure i_period_key is a string before splitting
    const periodKeyStr = String(Body.i_period_key);
    const [year, month] = periodKeyStr.split('-');
    ...
    this.http.post(url, Body, { headers }).subscribe(
      (response: any) => {
        if (response.data.length === 0) {
          ...
          const fileName = `Unmatched Transactions Listing Month - ${month}-${year}`;
          ...
          if (this.selectedFileType === 'xlsx') {
            ...
            // Add the year at the top of the PDF
            doc.setFontSize(12);
            const monthNames = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
            // Ensure `month` and `year` are treated as numbers.
            const monthNumber = Number(month);
            const yearNumber = Number(year);

            const monthName = monthNames[monthNumber - 1]; // Adjust for zero-based index
            doc.text(`Unmatched Transactions Listing - ${monthName} ${yearNumber} `, 14, 10);
