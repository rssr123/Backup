...
@Component({
  selector: 'app-refund-status-detailed',
  templateUrl: './refund-status.component.html',
  styleUrls: ['./refund-status.component.scss']
})
export class RefundStatusDetailedComponent implements OnInit {

  dlUrl = environment.apiUrl + '/api/rs/v1/generateRefundStatusReport';
  ...
  permList = perm.Reporting_and_Analysis_Refund_Status_Detailed 
  permListResponse = "";
  ...
  authorization() {
    ...
    .subscribe(
      (response: any) => {
        ...
        this.permViewAllow = this.permListResponse.includes(perm.Reporting_and_Analysis_Refund_Status_Detailed) ? 1 : 0;
