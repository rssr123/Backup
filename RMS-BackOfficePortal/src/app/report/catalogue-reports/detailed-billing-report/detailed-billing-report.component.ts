...
@Component({
  selector: 'app-detailed-billing-report',
  templateUrl: './detailed-billing-report.component.html',
  styleUrls: ['./detailed-billing-report.component.scss']
})
export class DetailedBillingReportComponent {

  dlUrl = environment.apiUrl + '/api/blr/v1/dblclassidreport';
  ...
  // paymentStatuses = [
  //   { value: 'PIP', label: 'Payment in Process' },
  //   { value: 'PP', label: 'Pending Payment' },
  //   { value: 'PE', label: 'Payment-Expired' },
  //   { value: 'P', label: 'Paid' },
  //   { value: 'F', label: 'Fail' },
  //   { value: 'EP', label: 'Email Pending' },
  //   { value: 'ES', label: 'Email Sent' },
  //   { value: 'EE', label: 'Email Expired' },
  //   { value: 'EF', label: 'Email Failed to Send' }
  // ];

  billingStatuses: any[] = [];

  // Configuring Permissions for User and roles variables
  permList = perm.Reporting_and_Analysis_Detailed_Billing_by_Class_ID
  permListResponse = "";
    ...
    this.authService.checkUserRole(this.authService.username, this.permList)
      .subscribe(
        (response: any) => {
          ...
          this.permViewAllow = this.permListResponse.includes(perm.Reporting_and_Analysis_Detailed_Billing_by_Class_ID) ? 1 : 0;
  ...
  dlbuttonPerms(): boolean {
    return !this.detailedBillingDateRange || this.detailedBillingDateRange.length !== 2 || this.dateCheck() || !this.tempClassId ;
  }
  ...
  downloadReport() {
    ...
    if (!this.tempClassId) {
      console.error("Class ID Selection is required.");
