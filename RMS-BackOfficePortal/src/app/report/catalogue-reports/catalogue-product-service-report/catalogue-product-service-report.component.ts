...
@Component({
  selector: 'app-catalogue-product-service-report',
  templateUrl: './catalogue-product-service-report.component.html',
  styleUrls: ['./catalogue-product-service-report.component.scss']
})
export class CatalogueProductServiceReportComponent implements OnInit{

  dlUrl = environment.apiUrl + '/api/cpsr/v1/prodserviceReport';
  ...
  permList = perm.Reporting_and_Analysis_Catalogue_Product_Service 
  permListResponse = "";
  ...
  authorization() {
    ...
    .subscribe(
      (response: any) => {
        ...
        this.permViewAllow = this.permListResponse.includes(perm.Reporting_and_Analysis_Catalogue_Product_Service) ? 1 : 0;
  ...
  datePreset(){
    this.productServiceDateRange = null;
    ...
    this.productServiceDateRange = [start, end];
  }
  ...
  dlbuttonPerms(): boolean {
    //return !this.productServiceDateRange || this.productServiceDateRange.length !== 2 || this.dateCheck() || !this.typeOfService;
    return !this.productServiceDateRange || this.productServiceDateRange.length !== 2 || this.dateCheck();
  }

  dateCheck() {
    if (this.productServiceDateRange?.length === 2) {
      const startDate = new Date(this.productServiceDateRange[0]);
      const endDate = new Date(this.productServiceDateRange[1]);
  ...
  // validateProductServiceDateRange(): boolean {
  //   if (!this.productServiceDateRange || this.productServiceDateRange.length !== 2) {
  //     return false;
  //   }
  //   const start = new Date(this.productServiceDateRange[0]);
  //   const end = new Date(this.productServiceDateRange[1]);
  //   return (end.getTime() - start.getTime()) <= 31536000000; // 12 months in milliseconds
  // }

  downloadReport() {
    // if (!this.typeOfService) {
    //   alert("Please select a Type of Service before generating the report.");
    //   return;
    // }

    var requestBody: {[k: string]: any} = {
      i_date_range_start: this.datepipe.transform(this.productServiceDateRange?.[0] ?? new Date(), 'yyyy-MM-dd') + ' 00:00:00',
      i_date_range_end: this.datepipe.transform(this.productServiceDateRange?.[1] ?? new Date(), 'yyyy-MM-dd') + ' 23:59:59',
      i_type_of_service: this.typeOfService,
  ...
  this.ParamService.getStates(
    ...
    (response: any) => {
      if (response.data && Array.isArray(response.data)) {
        this.paymentStatuses = (response.data as { param_cd: string, nm_en: string }[])
          .filter((item) => ['PIP', 'PP', 'PE', 'P', 'F'].includes(item.param_cd))
