...
@Component({
  selector: 'app-bank-in-slip',
  templateUrl: './bank-in-slip.component.html',
  styleUrls: ['./bank-in-slip.component.scss']
})
export class BankInSlipComponent {
  dlUrl = environment.apiUrl + '/api/otcreport/v1/bankinslip';
  ...
  bankInSlipNo: String | null = null;

	showResultAlert = false;

  permList = perm.Reporting_and_Analysis_Bank_In_Slip
  permListResponse = "";
  ...
  authorization() {
    ...
    .subscribe(
      (response: any) => {
        ...
        this.permViewAllow = this.permListResponse.includes(perm.Reporting_and_Analysis_Bank_In_Slip) ? 1 : 0;
  ...
  dateCheck() {
    ...
    var requestBody: {[k: string]: any} = {
      ...
      i_bank_in_slip_no: this.bankInSlipNo,
