...
@Component({
  selector: 'app-credit-control-case',
  templateUrl: './credit-control-case.component.html',
  styleUrls: ['./credit-control-case.component.scss']
})
export class CreditControlCaseComponent implements OnInit {
  ...
  paymentStatusCd: string | null = null;
  ...
  permCheck = perm.Credit_Control_SME_Task_Pickup + ',' + perm.Court_Order_View_Listing_Page;
  ...
  async fetchBillingRegListingData(){
    const url = environment.apiUrl + '/api/cc/v1/smerolelist';
  ...
  populate(){
    ...
    this.paymentStatusCd = this.taskInfo.pymt_status;
    ...
    this.loadParam(this.paymentStatusCd as string, 'cc-pymt-status');
  ...
  loadParam(paramCd: string, paramGrpNm: string){
      this.ParamService.getStates(environment.DefaultPage.toString(), environment.ItemPerPage.toString(), paramCd, paramGrpNm).subscribe((response: any) => {
        if (response.data.length >= 0 && response.data[0] != null && response.data[0].nm_en != null) {
          ...
          console.error('Invalid response format for ' + paramCd + '>' + paramGrpNm + ':', response);
      },
      (error) => {
        console.error('There was an error retrieving the status for ' + paramCd + '>' + paramGrpNm + ':', error);
      }
    );
  } 
  ...
  loadPermission() {
    this.authService.checkUserRole(this.authService.username, this.permCheck)
        .subscribe((response: any) => {
          ...
          this.permAllow = this.permCheckReturnString.includes(perm.Credit_Control_SME_Task_Pickup) ? 1 : 0;
          if(this.permAllow == 0)
            this.permAllow = this.permCheckReturnString.includes(perm.Court_Order_View_Listing_Page) ? 1 : 0;
          if (this.permAllow === 0) {
  ...
  assignToHistoryCheck(item: any): string{
    if(this.taskStatus == 'IP' || this.taskStatus == 'RQ' || this.taskStatus == 'CMI' || this.taskStatus == 'DMI' || this.taskStatus == 'I')
  ...
  showTaskAction(){
    if(((this.taskStatus == 'IP' || this.taskStatus == 'CMI' || this.taskStatus == 'DMI' || this.taskStatus == 'RQ' || this.taskStatus == 'I') && this.roles.includes('FINANCEADMIN')) 
  ...
  submitCheck(){
    if(this.remarks == null || this.remarks.length < 1)
      return true;
    if(this.taskStatus != 'IP' && this.taskStatus != 'PFSM' && this.taskStatus != 'PFH' && this.taskStatus != 'PSSS' && this.taskStatus != 'PL' && this.taskStatus != 'PCO' && this.taskStatus != 'CMI' && this.taskStatus != 'DMI' && this.taskStatus != 'RQ' && this.taskStatus != 'I')
      return true;
    else if(this.taskStatus == 'IP' || this.taskStatus == 'CMI' || this.taskStatus == 'DMI' || this.taskStatus == 'RQ' || this.taskStatus == 'I'){
  ...
  async submitForm(){
    ...
    if(this.taskStatus == 'IP' || this.taskStatus == 'PL' || this.taskStatus == 'PCO' || this.taskStatus == 'CMI' || this.taskStatus == 'DMI' || this.taskStatus == 'RQ' || this.taskStatus == 'I')
    ...
    if((this.taskStatus == 'IP' || this.taskStatus == 'CMI' || this.taskStatus == 'DMI' || this.taskStatus == 'RQ' || this.taskStatus == 'I') && this.selectedAction == 'assign')
      requestBody['i_group'] = this.selectedGroup;
    else if((this.taskStatus == 'IP' || this.taskStatus == 'CMI' || this.taskStatus == 'DMI' || this.taskStatus == 'RQ' || this.taskStatus == 'I') && (this.selectedAction == 'debit' || this.selectedAction == 'credit'))
