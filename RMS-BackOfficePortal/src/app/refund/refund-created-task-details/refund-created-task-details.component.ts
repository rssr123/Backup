...
@Component({
  selector: 'app-refund-created-task-details',
  templateUrl: './refund-created-task-details.component.html',
  styleUrls: ['./refund-created-task-details.component.scss']
})
export class RefundCreatedTaskDetailsComponent {
  ...
  stateHistory() {
    ...
    this.rtt_wf_id = history.state.rtt_wf_id;
    this.refund_type = history.state.refund_type;
  ...
  handleRequestRefund(): void {
    ...
    if (selectedItems.length > 0) {
      ...
      alert('Please select at least one item for a refund.');
    }
  }
    ...
    if (this.rms_type === 'Online') {
      ...
      this.http.post(url, requestBody, { headers }).subscribe(
        (response: any) => {
          ...
          this.rcpt_no = this.pgRCPTModel.length > 0 ? this.pgRCPTModel[0].rcptNo : '';
