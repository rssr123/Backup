...
@Service
@Slf4j
public class OTCReceiptReprintGenerator {
	...
	public OTCReceiptReprintGenerator(@Value("${jasper.rcpt.directory}") String rcpt_directory) {
	...
	public File generateReceipt(OTCollectionReceiptRequest r) throws IOException {
		if(r.getType().equals("pdf")) {
			...
			String uniqueFileName = String.format("SSM-Receipt-%s-%s.pdf", r.getRcpt().getRcptNo(), timestamp);
			File pdfFile = new File(rcpt_directory + uniqueFileName);
