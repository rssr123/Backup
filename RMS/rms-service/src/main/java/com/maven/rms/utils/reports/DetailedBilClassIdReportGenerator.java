...
@Service
@Slf4j
public class DetailedBilClassIdReportGenerator {
    private final String report = "/jasper/Detailed_Billing_ClassID_Report.jrxml";
	private String report_directory;

	@Autowired
	private DataSource ds;

    public DetailedBilClassIdReportGenerator(@Value("${jasper.reports.directory}") String report_directory) {
        this.report_directory = report_directory;
        log.info("Starting DetailedBilClassIdReportGenerator...");
    }

    public File generateReport(BillingReportRequest r) throws IOException, JRException, SQLException {
    ...
    private Map<String, Object> reportParameters(BillingReportRequest request) {
        final Map<String, Object> parameters = new HashMap<>();

        parameters.put("datestart",  request.getDateStart());
        parameters.put("dateend", request.getDateEnd());
        parameters.put("classidselection", request.getClassIdSelection());
