...
import com.maven.rms.utils.reports.UnmatchedAgingReportGenerator;

@DisallowConcurrentExecution
@Component
@Slf4j
public class UnmatchAging implements Job {
    //private static final Logger logger = LoggerFactory.getLogger(UpdateRICPWriteOff.class);

    @Autowired
    private SchedulerLogService schLogSvc;
    @Autowired
    private UnAgRepReqService umarrSvc;
    @Autowired
    private UnmatchedAgingReportGenerator umargGen;
	@Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        try {
        	UnmatchedAgingReportRequest request = umarrSvc.getPendingRequest();
        	if(request != null) {
	        	request.setStatus("I");
	        	request.setModified_by("system");
	        	umarrSvc.updateStatus(request);
        	}
        	
        	
            SchedulerLog newLog = new SchedulerLog("Report - Unmatched Aging Report.",
            ...
            if(request != null) {
            	File report = umargGen.generateReport(new ReportRequest("UM_Aging_" 
            			//+ request.getP_dt_req().format(DateTimeFormatter.ofPattern("yyyyMMddHHmm"))
            			+ request.getDtCreated().format(DateTimeFormatter.ofPattern("yyyyMMddHHmm"))
            			+ "." + request.getP_file_type(), request));
            	
            	if(report != null){
            		if(request.getP_email() == null || !request.getP_email().equals("")) {
            			String redirect = url + "/unmatched-aging";
            			String subject = "UNMATCHED AGING REPORT GENERATION - TASK ID  " + request.getTask_id();
            			  
            			String body = "Report: Unmatched Aging Report Generation"
                                  + "<br>Task ID: " + request.getTask_id()
                                  + "<br>Task Status: Success"
                                  + "<br><br>Dear Sir/Madam,<br><br>"
                                  + "This is to inform you that your Unmatched Aging Report Generation task has been SUCCESS."
                                  ...
                                  + "<br>Ini adalah untuk memaklumkan bahawa Unmatched Aging Report Generation Task anda telah BERJAYA."
            		...
            		umarrSvc.updateReportDet(request);
