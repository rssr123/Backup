...
@Service
@Slf4j
public class RefundSummaryStatusReportGenerator {
    private final String report = "/jasper/Refund_Summary_Status_Report.jrxml";
	private String report_directory;

	@Autowired
	private DataSource ds;

    public RefundSummaryStatusReportGenerator(@Value("${jasper.reports.directory}") String report_directory) {
        this.report_directory = report_directory;
        log.info("Starting RefundSummaryStatusReportGenerator...");
    }
    ...
    private Map<String, Object> reportParameters(RefundStatusRequest request) {
        ...
        parameters.put("division", request.getDivision());
        return parameters;
    }
}
