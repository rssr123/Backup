...
@Service
@Slf4j
public class CatalogueProductServiceReportGenerator {
    private final String report = "/jasper/Catalogue_Product_Service_Report.jrxml";
	private String report_directory;

	@Autowired
	private DataSource ds;

    public CatalogueProductServiceReportGenerator(@Value("${jasper.reports.directory}") String report_directory) {
        this.report_directory = report_directory;
        log.info("Starting CatalogueProductServiceReportGenerator...");
    }

    public File generateReport(CatalogueProductServiceRequest r) throws IOException, JRException, SQLException {
    ...
    private Map<String, Object> reportParameters(CatalogueProductServiceRequest request) {
        final Map<String, Object> parameters = new HashMap<>();

        parameters.put("datestart",  request.getDateStart());
        parameters.put("dateend", request.getDateEnd());
        parameters.put("typeofservice", request.getTypeOfService());
        parameters.put("paymentstatus", request.getPaymentStatus());
        parameters.put("entityno", request.getEntityNo());
        parameters.put("entitycustomerid", request.getEntityCustomerId());
        return parameters;
    }
}
