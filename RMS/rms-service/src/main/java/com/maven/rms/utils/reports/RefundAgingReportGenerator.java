...
@Service
@Slf4j
public class RefundAgingReportGenerator {
    private final String report = "/jasper/Refund_Aging_Report.jrxml";
	private String report_directory;

	@Autowired
	private DataSource ds;

    public RefundAgingReportGenerator(@Value("${jasper.reports.directory}") String report_directory) {
        this.report_directory = report_directory;
        log.info("Starting RefundAgingReportGenerator...");
    }
    ...
    private Map<String, Object> reportParameters(RefundStatusRequest request) {
        ...
        parameters.put("refundno", request.getRefundNo());
        return parameters;
    }
}
