...
@Service
@Slf4j
public class ReportUTLGenerator {

    //private static final Logger logger = LoggerFactory.getLogger(PaymentCollectionReportGenerator.class);
    private final String report_template_path = "/jasper/Unmatched_Transaction_Listing.jrxml";
    ...
    public ReportUTLGenerator(@Value("${jasper.reports.directory}") String report_directory){
    ...
    public File generateReport(ReportRequest r) throws IOException, JRException, SQLException{
		...
		// try {
			return r.getFileType().equals("pdf") ? generatePDFReport(report, loadTemplate(), reportParameters(r)) :
				r.getFileType().equals("xlsx") ? generateXLSXReport(report, loadTemplate(), reportParameters(r)) :
				r.getFileType().equals("xls") ? generateXLSXReport(report, loadTemplate(), reportParameters(r)) : 
				r.getFileType().equals("csv") ? generateCSVReport(report, loadTemplate(), reportParameters(r)) : null;
		// } catch (IOException e) {
	...
	public File generateCSVReport(File file, JasperReport report, Map<String, Object> reportParameters) throws IOException, JRException, SQLException {
   	...
   	private JasperReport loadTemplate() throws JRException {
        ...
        // }
        // catch(final Exception e){
        //     throw new JRException("UNKNOWN REPORT TYPE: !");
        // }
   	...
   	private Map<String, Object> reportParameters(ReportRequest r) {
		...
		parameters.put("pgtxnid", r.getPgTxnId());
