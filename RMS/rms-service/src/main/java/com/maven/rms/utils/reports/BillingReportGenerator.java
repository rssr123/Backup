...
public class BillingReportGenerator {
    private final String report = "/jasper/Billing_Report.jrxml";
	private String report_directory;

	@Autowired
	private DataSource ds;

    public BillingReportGenerator(@Value("${jasper.reports.directory}") String report_directory) {
        this.report_directory = report_directory;
        log.info("Starting BillingReportGenerator...");
    }
    ...
    private Map<String, Object> reportParameters(BillingReportRequest request) {
        ...
        parameters.put("billingcategory", request.getBillingCategory());
        parameters.put("paymentstatus", request.getPaymentStatus());
        parameters.put("billingno", request.getBillingNo());
