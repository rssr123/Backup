...
@Service
@Slf4j
public class OTCCollectionPlusReportGenerator {
    private final String report = "/jasper/OTC_Collection_Plus.jrxml";
	...
	public OTCCollectionPlusReportGenerator(@Value("${jasper.reports.directory}") String report_directory){
		this.report_directory = report_directory;
		log.info("Starting OTCCollectionPlusReportGenerator...");
	}
	...
	private Map<String, Object> reportParameters(OTCReportRequest request) {
		...
		if(request.getField1() != null)
			parameters.put("feedetailid", request.getField1());

        if(request.getField2() != null)
            parameters.put("ledgercode", request.getField2());
