...
@Service
@Slf4j
public class MTTPGWatermarkedReceiptGenerator {
	...
	private static final String copy_watermark_path = "/jasper/images/copy_watermark.png";
	private final String receipt_template_path = "/jasper/SSM_Receipt_Watermarked.jrxml";
	private String rcpt_directory;
	
	@Autowired
	private StoreProcedureService spService;

	public MTTPGWatermarkedReceiptGenerator(@Value("${jasper.rcpt.directory}") String rcpt_directory) {
		this.rcpt_directory = rcpt_directory;
		log.info("Starting MTTPGReceiptGeneratorService...");
	}
	
	public File generateReceipt(ReceiptRequest r, int watermark) throws IOException {
		if(r.getType().equals("pdf")) {
			File pdfFile = new File(rcpt_directory + "SSM-Receipt-" + r.getRcpt().getRcptNo() + ".pdf");
			
			try(FileOutputStream pos = new FileOutputStream(pdfFile)){
				final JasperReport report = loadTemplate();
				final Map<String, Object> parameters = receiptParameters(r.getPG(), r.getMtt(), r.getRcpt(), r.getItemList(), watermark);
			...
			return generateXLSX(r.getRcpt().getRcptNo(), receiptParameters(r.getPG(), r.getMtt(), r.getRcpt(), r.getItemList(), watermark));
		else
			return null;
	}
	
	private Map<String, Object> receiptParameters(MTTPG pG, OnlinePayment mtt, MTTRCPT rcpt, List<OnlinePaymentItem> itemList, int watermark) {
		...
		switch(watermark) {
			case 1:
				...
				break;
			case 2:
				parameters.put("watermark", getClass().getResourceAsStream(copy_watermark_path));
				break;
			default:
				parameters.put("watermark", getClass().getResourceAsStream(cancel_watermark_path));
		}
