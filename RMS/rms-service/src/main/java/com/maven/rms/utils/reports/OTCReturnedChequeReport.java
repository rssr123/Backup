...
@Service
@Slf4j
public class OTCReturnedChequeReport {
    private final String report = "/jasper/OTC_Returned_Cheque.jrxml";
	...
	public OTCReturnedChequeReport(@Value("${jasper.reports.directory}") String report_directory){
		this.report_directory = report_directory;
		log.info("Starting OTCReturnedChequeReport...");
	}
	...
	private Map<String, Object> reportParameters(OTCReportRequest request) {
        ...
        if(request.getField2() != null)
            parameters.put("receiptno", request.getField2());

        if(request.getField3() != null)
            parameters.put("chequeno", request.getField3());

        if(request.getField4() != null)
            parameters.put("customername", request.getField4());

        if(request.getField5() != null)
            parameters.put("entityno", request.getField5());

        if(request.getField6() != null)
            parameters.put("nonbillingno", request.getField6());

        if(request.getField7() != null)
            parameters.put("nonbillingstatus", request.getField7());
