...
@Service
@Slf4j
public class BankInSlipReportGenerator {
    private final String report = "/jasper/Bank_In_Slip.jrxml";
	...
	public BankInSlipReportGenerator(@Value("${jasper.reports.directory}") String report_directory){
		this.report_directory = report_directory;
		log.info("Starting BankInSlipReportGenerator...");
	}
	...
	private Map<String, Object> reportParameters(OTCReportRequest request) {
        ...
        if(request.getField2() != null)
            parameters.put("bankinslipno", request.getField2());
